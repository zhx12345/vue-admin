	...
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" stripe>
				...
				<el-table-column label="分类" prop="has_categories.name" width="150"></el-table-column>
				<el-table-column label="通知名称" prop="title" width="1000"></el-table-column>
...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.content.notices.list,
			selection: [],
			search: {
				title: ""
			}
	...
	methods: {
		...
		async table_del(row){
			...
			var res = await this.$API.content.notices.delete.delete(reqData.id);
			if(res.code == 200){
			...
			this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？如果删除项中含有子集将会被一并删除`, '提示', {
				...
				this.selection.forEach(item => {
					this.$refs.table.tableData.forEach((itemI, indexI) => {
						if (item.id === itemI.id) {
							// 删除数据
							this.$API.content.notices.delete.delete(itemI.id);
							// 删除页面显示
