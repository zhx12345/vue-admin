			...
			<el-container>
				<el-header style="height: auto;display: block;">
					<div class="user-info-top">
						<el-avatar :size="70" :src="user.avatar" v-if="user.avatar != null"></el-avatar>
						<el-avatar :size="70" v-else>{{ user.userNameF }}</el-avatar>
						<h3>({{ user.user_name }}){{ user.name }}</h3>
	...
	export default {
		...
		methods: {
			...
			async userInfo(){
				...
				userInfo.userNameF = userInfo.user_name.substring(0,1)
				this.user = userInfo
			},
			openPage(item){
