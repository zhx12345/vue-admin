...
<script>
	...
	export default {
		...
		methods: {
			...
			async save(){
				this.loading = true
				var res = await this.$API.system.menu.edit.put(this.form)
				this.loading = false
				if(res.code == 200){
			...
			setData(data, pid){
				this.form = data
				this.form.meta.hidden = this.form.meta.hidden == 1 ? true : false
				this.form.meta.hidden_breadcrumb = this.form.meta.hidden_breadcrumb == 1 ? true : false
