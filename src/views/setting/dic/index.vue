				...
				<scTable ref="table" :apiObj="listApi" row-key="id" :params="listApiParams" @selection-change="selectionChange" stripe :paginationLayout="'prev, pager, next'">
					...
					<el-table-column label="是否有效" prop="yx" width="100">
						<template #default="scope">
							<el-tag type="success" v-if="scope.row.yx == 1">使用中</el-tag>
		...
		methods: {
			...
			dicDel(node, data){
				this.$confirm(`确定删除 ${data.name} 项吗？`, '提示', {
					...
					//删除
					this.$API.system.dic.typeDelete.delete(data.id);
			...
			async table_del(row, index){
				...
				var res = await this.$API.system.dic.delete.delete(reqData.id);
				if(res.code == 200){
				...
				this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？`, '提示', {
					...
					this.selection.forEach(item => {
						this.$refs.table.tableData.forEach((itemI, indexI) => {
							if (item.id === itemI.id) {
								// 删除数据
								this.$API.system.dic.delete.delete(itemI.id);
								// 删除页面显示
